"""
Training script for nail point cloud part segmentation.
Adapted from examples/shapenetpart/main.py for binary nail segmentation.
"""
import argparse
import yaml
import os
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('NailSeg part segmentation training')
    ...
    # logger
    # init log dir
    cfg.task_name = args.cfg.split('.')[-2].split('/')[-2]  # task/dataset name, \eg nailseg
    cfg.cfg_basename = args.cfg.split('.')[-2].split('/')[-1]  # cfg_basename, \eg pointnext-s
    tags = [
